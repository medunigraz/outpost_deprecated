DDL
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""FUNKTIONEN_V"" (""FUNK_NR"", ""FUNK_BEZ"", ""FUNK_NAME_W"", ""FUNK_NAME_M"", ""FUNK_GRUPPE"", ""FUNK_LEITER"") AS 
  select f.NR funk_nr
...
where f.FTYP_GRP_NR = fg.NR
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""GEBAEUDE_V"" (""GEBAEUDE_NR"", ""NAME"", ""KURZBEZEICHNUNG"", ""STRASSE_HAUSNUMMER"") AS 
...
;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""INT_PROFIL_BILDER_IDS_V"" (""PERSON_NR"", ""DOCUMENT_NR"") AS 
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""LV_GRP_STUD_V"" (""GRP_NR"", ""STUD_NR"") AS 
...
;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""LV_GRP_TERM_V"" (""LV_GRP_NR"", ""PERS_NR"", ""TERMIN_NR"", ""LV_BEGINN"", ""LV_ENDE"", ""LERNEINHEIT"", ""RAUM_NR"", ""RAUM_NUMMER"", ""RAUM_BEZ"", ""GEBAEUDE"", ""STOCKWERK"") AS 
...
;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""LV_GRP_V"" (""GRP_NR"", ""LV_NR"", ""GRP_NAME"") AS 
  select g.NR          grp_nr
      ,g.STP_SP_NR    lv_nr
      ,g.NAME        grp_name
...
;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""LV_TERM_RAUM_V"" (""TERMIN_NR"", ""RAUM_NR"", ""RAUM_NUMMER"", ""RAUM_BEZ"", ""GEBAEUDE"", ""STOCKWERK"") AS 
...
;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""LV_V"" (""LV_NR"", ""LV_LVNR"", ""LV_TITEL"", ""LV_TYP"", ""LV_TYP_LANG"", ""LV_STUDJAHR"", ""LV_SEMESTER"", ""SEMESTER_BEZEICHNUNG"") AS 
  SELECT s.NR         lv_nr
      ,s.LVNR       lv_lvnr
      ,s.TITEL      lv_titel
      ,sa.NAME_kurz lv_typ
...
;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""MITTEILUNGSBLATT_V"" (""NR"", ""STUDIENJAHR"", ""STUECK"", ""AUSGABEDATUM"", ""ZUSATZ_INFO"", ""LINK"") AS 
  SELECT a.NR
      ,a.STUDIENJAHR
      ,a.STUECK
      ,a.AUSGABEDATUM
...
;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""ORGANISATIONEN_IDS_V"" (""NR"", ""TYP"") AS 
  SELECT --Med Uni Graz
  o1.NR,
  'V' as Typ
...
select
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""ORGANISATIONEN_V"" (""NR"", ""NAME_KURZ"", ""ORG_NAME"", ""BASISORGANISATION"", ""TYP"", ""ADRESSE"", ""EMAIL_ADRESSE"", ""TELEFON_NUMMER"", ""WWW_HOMEPAGE"", ""SORT_HIERARCHIE"") AS 
  Select
  o.NR as NR,
...
order by o.SORT_HIERARCHIE;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""PERSONEN_ORGANISATIONEN_V"" (""PERSON_NR"", ""ORG_NR"", ""PERS_FAMNAM"", ""PERS_VORNAME"") AS 
  select 
        DISTINCT
        p.PERS_NR person_nr
       ,po.ORG_NR
...
where
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""PERSONEN_PROFILBILDER_DATEN_V"" (""PERSON_NR"", ""CONTENT"") AS 
  select pr.PERSON_NR, store.CONTENT
   from INT_PROFIL_BILDER_IDS_V pr
      , TUG_NEW.DOCUMENTS_STORE store
  where pr.DOCUMENT_NR = store.nr;

  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""PERSONEN_RAUM_ALLE_V"" (""NR"", ""VORNAME"", ""FAMILIENNAME"", ""TITEL"", ""RAUM_NR"") AS 
...
GROUP BY pers.nr, pers.VORNAME, pers.FAMILIENNAME, pers.TITEL;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""PERSONEN_RAUM_V"" (""NR"", ""VORNAME"", ""FAMILIENNAME"", ""TITEL"", ""RAUM_NR"", ""RAUM_BEZ"", ""GEBAEUDE_NR"", ""GEBAEUDE"", ""STOCKWERK_NR"", ""STOCKWERK"", ""RAUM"", ""RAUM_NUMMER"", ""FLAECHE"", ""HOEHE"", ""ORG_NR"", ""ORGANISATION"", ""RAUMTYP"") AS 
  SELECT DISTINCT 
       p.NR,
       p.VORNAME,
       p.FAMILIENNAME,
...
FROM CO_LOC_API.PERSONEN_RAUM_ALLE_V p, CO_LOC_API.RAUM_V v where v.RAUM_NR = p.RAUM_NR;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""PERSON_I_V"" (""PERS_NR"", ""PERS_FAMNAM"", ""PERS_VORNAME"", ""PERS_TITEL"", ""PERS_SEX"", ""PERS_BENUTZERNAME"", ""PERS_SPRECHSTUNDE"", ""PERS_ZUSATZ_INFO"", ""PERS_PROFILBILD"", ""PERS_EMAIL"") AS 
  select DISTINCT  
      p.nr           pers_nr
...
where p.NR = bk.PERSON_NR
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""PERSON_V"" (""PERS_NR"", ""PERS_FAMNAM"", ""PERS_VORNAME"", ""PERS_TITEL"", ""PERS_SEX"", ""PERS_BENUTZERNAME"", ""PERS_SPRECHSTUNDE"", ""PERS_ZUSATZ_INFO"", ""PERS_PROFILBILD"", ""RAUM_NR"", ""PERS_EMAIL"") AS 
  Select p.pers_nr, 
        p.pers_famnam,
        p.pers_vorname,
        p.pers_titel,
...
GROUP BY p.pers_nr, 
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""PERS_ORG_FUNK_V"" (""PERS_NR"", ""ORG_NR"", ""FUNK_NR"") AS 
...
where po.PERSON_NR = p.PERS_NR
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""RAUM_TYPEN_V"" (""NR"", ""NAME_DE"", ""NAME_EN"") AS 
  SELECT 
    rt.nr NR, rt.name.de NAME_DE, rt.name.en NAME_EN
FROM 
    tug_new.Raum_typen_lt rt;

  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""RAUM_V"" (""RAUM_NR"", ""RAUM_BEZ"", ""GEBAEUDE_NR"", ""GEBAEUDE"", ""STOCKWERK_NR"", ""STOCKWERK"", ""RAUM"", ""RAUM_NUMMER"", ""FLAECHE"", ""HOEHE"", ""ORG_NR"", ""ORGANISATION"", ""RAUMTYP"") AS 
  select r.nr Raum_nr
...
--   and g.KURZBEZEICHNUNG like 'MC%'
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""STOCKWERK_V"" (""NR"", ""KURZBEZEICHNUNG"", ""NAME"") AS 
...
;
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""STUD_V"" (""STUD_NR"", ""STUD_MNR"", ""STUD_FAMNAM"", ""STUD_VORNAME"", ""STUD_AKADGRAD"", ""STUD_SEX"", ""STUD_MIFARE"") AS 
  select distinct 
      sp.NR               stud_nr
...
where sp.NR = st.ST_PERSON_NR
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""VERANSTALTUNGEN_HEUTE_V"" (""PK_INTWB"", ""PK_LV"", ""PK_VERANST"", ""REIHUNG"", ""TYP"", ""NUMMER"", ""TITEL"", ""DATUM"", ""ZEIT_VON"", ""ZEIT_BIS"", ""PK_GEB"", ""GEBAEUDE"", ""PK_RAUM"", ""RAUM"", ""RAUM_BEZ"", ""TERMINART"", ""ANZEIGE_BIS"") AS 
...
--      ,p.FAMILIENNAME||' '||p.VORNAME vortragender
    ...
    -- anzeigen bis von-zeit plus 20 minuten:
      ,to_date( to_char( t.datum_am, 'dd.mm.yyyy ')||to_char( t.ZEIT_VON, 'hh24:mi'), 'dd.mm.yyyy hh24:mi') + 30/1440 anzeige_bis
from tug_new.pu_va_veranstaltungen_v v
    ,tu

  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""VERTEILERLISTE_PERSON_V"" (""PROFIL_NR"", ""PERS_NR"", ""PERS_BENUTZERNAME"") AS 
  select
        vtl.profil_nr,
        vtl.pers_nr,
  ...
  CREATE OR REPLACE FORCE VIEW ""CO_LOC_API"".""VERTEILERLISTE_V"" (""PROFIL_NR"", ""PROFIL_NAME"") AS 
  SELECT DISTINCT
        vte.profil_nr,
        b.profil_name
    FROM
